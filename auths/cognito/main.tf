...
resource "aws_cognito_user_pool" "user_pool" {
  ...
  admin_create_user_config {
    ...
    invite_message_template {
      email_message = var.email_message != "" ? var.email_message : <<EOF
        ...
        <p></p>
        <p>Please login with this URL https://argocd.solutionsconsulting.net/</p>
        <p>Click on LOG IN VIA COGNITO. It will get redirected to the AWS Cognito login and change your password.</p>
        </p>
        <p></p>
        <p>Sincerely Yours,</p>
        <p>The ${var.name} Team</p>
        EOF
  ...
  dynamic "schema" {
    ...
    content {
      ...
      required                 = try(attribute.value.required, true)
...
resource "aws_cognito_identity_pool" "identity_pool" {
  identity_pool_name               = format("%s-identity-pool", module.labels.id)
  allow_unauthenticated_identities = true

  dynamic "cognito_identity_providers" {
    for_each = var.enable_cognito_identity_providers ? aws_cognito_user_pool_client.client : []

    content {
      client_id               = cognito_identity_providers.value.id
      provider_name           = "cognito-idp.${local.aws_region}.amazonaws.com/${cognito_identity_providers.value.user_pool_id}"
...
locals {
  groups_default = [
    {
      ...
      # The creation is part of the module if not then you parse an existing role_arn
      # If existing then role_arn    = var.user_group_role_arn
  ...
  groups_parsed = [for e in var.user_groups : {
    ...
    # The creation is part of the module if not then you parse an existing role_arn
    # If existing then # role_arn    = lookup(e, "role_arn", null)
    }
  ]
...
resource "aws_iam_role" "group_role" {
  name                = "${module.labels.id}-user-group-role"
  assume_role_policy  = data.aws_iam_policy_document.trust_relationship.json
  managed_policy_arns = ["arn:aws:iam::aws:policy/AmazonCognitoReadOnly"]
}
...
#   group_name   = var.enable_cognito_user_in_group ? aws_cognito_user_group.main[0].name : null
#   username     = var.enable_cognito_user_in_group ? aws_cognito_user.users[0].username : null
# }

# variable "enable_cognito_user_in_group" {
#   description = "Flag to enable or disable the creation of aws_cognito_user_in_group"
#   type        = bool
#   default     = false
# }