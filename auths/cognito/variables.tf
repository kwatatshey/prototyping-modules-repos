...
variable "attributes_require_verification_before_update" {
  type        = list(any)
  default     = ["email"]
  description = "Attributes that are required to be verified before the user is updated"
}
...
# If existing then put uncomment this variable
...
variable "users" {
  ...
  type = map(
    object({
      email    = string
      username = string # unique, non-email identifier
    })
  )
}
...
variable "enable_cognito_identity_providers" {
  description = "Flag to enable or disable the creation of cognito_identity_providers"
  type        = bool
}
